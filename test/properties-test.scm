;;; This file is part of Integer Sequences, a library for recreational
;;; number theory in MIT Scheme.
;;; Copyright 2007-2009 Alexey Radul.
;;;
;;; Integer Sequences is free software; you can redistribute it and/or
;;; modify it under the terms of the GNU Affero General Public License
;;; as published by the Free Software Foundation; either version 3 of
;;; the License, or (at your option) any later version.
;;; 
;;; This code is distributed in the hope that it will be useful,
;;; but WITHOUT ANY WARRANTY; without even the implied warranty of
;;; MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
;;; GNU General Public License for more details.
;;; 
;;; You should have received a copy of the GNU Affero General Public
;;; License along with Integer Sequences; if not, see
;;; <http://www.gnu.org/licenses/>.

(declare (usual-integrations))

(in-test-group
 properties

 (define-each-check
  (equal? 4 (increment 3))
  (equal? 2 (decrement 3))
  (equal? 6 (double 3))
  (equal? 12 (triple 4))
  (equal? 5 (halve 10))
  (equal? 10 (sum '(1 2 3 4)))
  (equal? 24 (product '(1 2 3 4)))
  (equal? 24 (factorial 4))
  (factorial? 24)
  (not (factorial? 25))
  (equal? '(1 2 6 24) (stream->list 4 (the-factorials)))
  (equal? 10 (choose 2 5))
  (equal? 55 (fibonacci 10))
  (fibonacci? 55)
  (not (fibonacci? 70))
  (equal? -55 (fibonacci -10))
  (equal? 34 (fibonacci -9))

  (equal? 2 (smallest-divisor 6))
  (equal? 5 (smallest-divisor 175))
  (prime? 17)
  (not (prime? 16))
  (equal? 11 (prime 5))
  (not (composite? 17))
  (composite? 16)
  (coprime? 6 35)
  (not (coprime? 6 10))
  (equal? 2 (euler-phi 6))
  (semiprime? 10)
  (not (semiprime? 7))
  (not (semiprime? 175))
  (square-free? 6)
  (not (square-free? 12))
  (equal? '(5 5 7) (prime-factors 175))
  (equal? '(1 2 3 4 5 6 10 12 15 20 30 60) (divisors 60))
  (equal? '(1 2 4 5 10 20 25 50 100) (divisors 100))
  (equal? '(1 2 3 4 5 6 10 12 15 20 30) (proper-divisors 60))
  (equal? '(1) (proper-divisors 101))
  (equal? 108 (sigma 60))
  (perfect? 6)
  (perfect? 28)
  (not (perfect? 27))
  (equal? '(6 28 496) (stream->list 3 (the-perfects)))
  (abundant? 60)
  (not (abundant? 7))
  (not (abundant? 6))
  (equal? '(12 18 20 24) (stream->list 4 (the-abundants)))
  (not (deficient? 60))
  (deficient? 7)
  (not (deficient? 6))
  (equal? 25 (deficient 20))
  (amicable? 220)
  (not (amicable? 221))
  (not (amicable? 1))
  (equal? '(220 284) (stream->list 2 (the-amicables)))
  (aspiring? 25)
  (not (aspiring? 26))
  (not (aspiring? 276)) ; Well, we don't really know...
  (equal? '(25 95 119 143) (stream->list 4 (the-aspirings)))
  (equal? 2047 (mersenne 5))
  (mersenne? 2047)
  (not (mersenne? 2000))
  (equal? 2310 (primorial 5))
  (primorial? 2310)
  (not (primorial? 2000))
  (equal? 17280 (compositorial 5))
  (compositorial? 17280)
  (not (compositorial? 17000))

  (equal? 3 (integer-log 3 27))
  (< 3 (integer-log 3 28) 4)
  (power-of? 3 27)
  (not (power-of? 2 27))

  (equal? 36 (square 6))
  (square? 36)
  (not (square? 35))
  (equal? 27 (cube 3))
  (cube? 27)
  (not (cube? 26))
  (equal? 15 (triangle 5))
  (triangle? 15)
  (not (triangle? 20))
  (triangle? 21)
  (equal? 5 (triangle-root 15))
  (< 5 (triangle-root 16) 6)
  (equal? 5 (count-triangles 3 25))
  (equal? '(1 3 6 10 15) (stream->list 5 (the-triangles)))
  (equal? '(10 15 21 28 36) (stream->list 5 (triangles-from 10)))
  (equal? '(15 10 6 3 1) (stream->list (triangles-down-from 15)))
  (equal? '(10 15 21 28 36) (stream->list (triangles-between 9 37)))
  (equal? '(36 28 21 15 10) (stream->list (triangles-between-down 9 37)))
  ;; TODO Other figurate numbers

  (equal? '(1 2 3 4) (number->digits 1234))
  (equal? '(1 0 1) (number->bits 5))
  (equal? 1234 (digits->number '(1 2 3 4)))
  (pandigital? 1234567890)
  (not (pandigital? 3257454))
  (equal? 3 (bitcount 7))
  (evil? 6)
  (not (evil? 7))
  (odious? 7)
  (not (odious? 6))
  (multidigit? 23)
  (not (multidigit? 5))
  (palindrome? 1)
  (palindrome? 55)
  (palindrome? 141)
  (not (palindrome? 142))
  (emirp? 17)
  (emirp? 71)
  (not (emirp? 53))
  (emirpimes? 15)
  (emirpimes? 51)
  (not (emirpimes? 17))
  (not (emirpimes? 91))
  (strobogrammatic? 619)
  (not (strobogrammatic? 99))
  (apocalyptic-power? 157)
  (not (apocalyptic-power? 158))
  (smith? 22)
  (not (smith? 21))
  (hoax? 22)
  (not (hoax? 21))
  (equal? 111 (repunit 3))
  (repunit? 111)
  (not (repunit? 112))
  (equal? '(1 11 111 1111) (stream->list 4 (the-repunits)))
  (repdigit? 444)
  (not (repdigit? 454))
  (undulating? 343434)
  (not (undulating? 343437))
  (equal? '(101 111 121) (stream->list 3 (the-undulatings)))

  (equal? '("one" "thousand" "two" "hundred" "fifteen")
          (number->words 1215))
  (equal? 28 (letter-count 1215))
  (equal? "MCCXV" (number->roman-numerals 1215))))
