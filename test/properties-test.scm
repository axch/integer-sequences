...
(in-test-group
 ...
 (define-each-check
  ...
  (fibonacci? 1)
  (equal? 1 (fibonacci-root 1)) ; Not 2, even though next example
  (equal? 1 (fibonacci 2))
  (equal? 3 (fibonacci-root 2))
  (equal? 0 (count-fibonaccis 1 1)) ; Exclusive of upper bound
  (equal? 2 (count-fibonaccis 1 2)) ; 1,1 counts as two Fibonaccis
  (equal? 4 (count-fibonaccis 1 4)) ; 1,1 counts as two Fibonaccis
